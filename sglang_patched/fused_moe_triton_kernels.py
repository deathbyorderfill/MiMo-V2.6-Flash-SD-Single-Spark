...
def fused_moe_kernel_gptq_awq(
    ...
    if use_int4_w4a16:
        ...
        # 4 two-bit codes per byte along K, LSB first
        b_ptrs = (
            b_ptr
            + off_experts * stride_be
    ...
    for k in range(0, tl.cdiv(K, BLOCK_SIZE_K)):
        ...
        if use_fp2_w2a16:
            # lm20 codebook: bit0 -> magnitude {0.5, 2.0}, bit1 -> sign
            b_mag = tl.where((b & 1) != 0, 2.0, 0.5)
            b_val = tl.where((b & 2) != 0, -b_mag, b_mag)
            b = (b_val * b_scale).to(compute_type)
        elif has_zp:
...
@triton.jit
def fused_moe_kernel_fp2(
    a_ptr, b_ptr, c_ptr, s_ptr, g_ptr, topk_weights_ptr, sorted_token_ids_ptr, expert_ids_ptr,
    num_tokens_post_padded_ptr,
    N: tl.constexpr, K: tl.constexpr, EM, num_valid_tokens,
    stride_am, stride_ak, stride_be, stride_bn, stride_bk, stride_cm, stride_cn,
    stride_se, stride_sn, stride_sk, stride_ge, stride_gn,
    BLOCK_SIZE_M: tl.constexpr, BLOCK_SIZE_N: tl.constexpr, BLOCK_SIZE_K: tl.constexpr,
    GROUP_SIZE_M: tl.constexpr, MUL_ROUTED_WEIGHT: tl.constexpr, top_k: tl.constexpr,
    compute_type: tl.constexpr, filter_expert: tl.constexpr, FP2_GROUP: tl.constexpr = 16,
):
    """NVFP2/lm20 W2A16 grouped MoE GEMM (opt3). B: [E, N, K//4] uint8, 4 codes per
    byte along K (LSB first): bit0 -> magnitude {0.5, 2.0}, bit1 -> sign. S: bf16
    [E, N, K//16] = e4m3 block scale * per-tensor scale. Each packed byte is expanded
    into 4 K-planes (k = 4j + c); A is read with the matching stride-4 column pattern,
    so 4 dots of K/4 each replace one dot of K. Scales are loaded once per 16-group and
    expanded x4 in registers (the generic gptq/awq kernel loads them per element)."""
    pid = tl.program_id(axis=0)
    num_pid_m = tl.cdiv(EM, BLOCK_SIZE_M)
    num_pid_n = tl.cdiv(N, BLOCK_SIZE_N)
    num_pid_in_group = GROUP_SIZE_M * num_pid_n
    ...
    off_experts = tl.load(expert_ids_ptr + pid_m).to(tl.int64)
    if filter_expert and off_experts == -1:
        write_zeros_to_output(c_ptr, stride_cm, stride_cn, pid_n, N, offs_token, token_mask,
                              BLOCK_SIZE_M, BLOCK_SIZE_N, compute_type)
        return

    offs_bn = (pid_n * BLOCK_SIZE_N + tl.arange(0, BLOCK_SIZE_N).to(tl.int64)) % N
    KB: tl.constexpr = BLOCK_SIZE_K // 4
    KG: tl.constexpr = BLOCK_SIZE_K // FP2_GROUP  # mimo26: scale group 16 or 32
    KPG: tl.constexpr = FP2_GROUP // 4
    offs_kb = tl.arange(0, KB)
    offs_kg = tl.arange(0, KG)
    a_base = a_ptr + (offs_token[:, None] // top_k) * stride_am + (4 * offs_kb[None, :]) * stride_ak
    b_ptrs = b_ptr + off_experts * stride_be + offs_kb[:, None] * stride_bk + offs_bn[None, :] * stride_bn
    s_ptrs = s_ptr + off_experts * stride_se + offs_kg[:, None] * stride_sk + offs_bn[None, :] * stride_sn

    accumulator = tl.zeros((BLOCK_SIZE_M, BLOCK_SIZE_N), dtype=tl.float32)
    for k in range(0, tl.cdiv(K, BLOCK_SIZE_K)):
        packed = tl.load(b_ptrs)
        s = tl.load(s_ptrs).to(tl.float32)
        s4 = tl.reshape(tl.broadcast_to(s[:, None, :], (KG, KPG, BLOCK_SIZE_N)), (KB, BLOCK_SIZE_N))
        for c in tl.static_range(4):
            a = tl.load(a_base + c * stride_ak, mask=token_mask[:, None], other=0.0)
            code = (packed >> (2 * c)) & 0x3
            v = tl.where((code & 1) != 0, 2.0, 0.5)
            v = tl.where((code & 2) != 0, -v, v)
            b = (v * s4).to(compute_type)
            accumulator = tl.dot(a, b, acc=accumulator)
        a_base += BLOCK_SIZE_K * stride_ak
        b_ptrs += KB * stride_bk
        s_ptrs += KG * stride_sk

    # exact per-tensor scale (fp32, per expert and output row) applied once in the
    # epilogue; the in-loop scale is the raw e4m3 block scale, so no rounding is added
    g = tl.load(g_ptr + off_experts * stride_ge + offs_bn * stride_gn).to(tl.float32)
...
def invoke_fused_moe_kernel(
    ...
    if fuse_swiglu:
        ...
        assert not (use_fp8_w8a8 or use_int8_w8a8 or use_int8_w8a16 or use_int4_w4a16 or use_fp2_w2a16)
        assert bias is None
    ...
    if use_fp8_w8a8:
        ...
        if block_shape is None:
            ...
            assert triton.cdiv(B.shape[-2], block_n) == B_scale.shape[-2]
            assert triton.cdiv(B.shape[-1], block_k) == B_scale.shape[-1]
    elif use_int8_w8a16 or use_int4_w4a16 or use_fp2_w2a16:
        assert B_scale is not None
    ...
    if use_fp2_w2a16:
        # fused_moe_kernel_fp2 writes C with plain stores over the full
        # (M*topk, N) extent, unsorted. Every flavour that changes the output
        # contract (all-reduce combine into an (M, hidden) buffer, half-width
        # swiglu buffer, LoRA add/mask, sorted C) would be an out-of-bounds or
        # silently wrong write, so refuse them here instead of dispatching.
        assert not fuse_sum_all_reduce, "fp2 kernel does not implement fuse_sum_all_reduce"
        assert not fuse_swiglu, "fp2 kernel does not implement fuse_swiglu"
        assert not (fuse_add_to_output or mask_output or lora_preserve_base), "fp2 kernel does not implement the LoRA output paths"
        assert not c_sorted, "fp2 kernel writes C in token order (c_sorted=False)"
        assert B_scale is not None and B_scale.ndim == 3 and bias is None
        assert block_shape is not None and block_shape[1] in (16, 32)
        # A.shape[1] is the true K; B.shape[2] is K//4 (packed)
        assert A.shape[1] % config["BLOCK_SIZE_K"] == 0 and config["BLOCK_SIZE_K"] % block_shape[1] == 0, (A.shape, config)
        # B_zp slot carries the fp32 per-tensor scale [E, N] for the fp2 path
        assert B_zp is not None and B_zp.ndim == 2 and B_zp.dtype == torch.float32, "fp2 needs fp32 per-tensor scales in the zp slot"
        fused_moe_kernel_fp2[grid](
            A, B, C, B_scale, B_zp, topk_weights, sorted_token_ids, expert_ids,
            num_tokens_post_padded,
            B.shape[1], A.shape[1], sorted_token_ids.shape[0], topk_ids.numel(),
            ...
            filter_expert=filter_expert, FP2_GROUP=block_shape[1], **config,
        )
        return

    if (
        (use_int8_w8a16 or use_int4_w4a16 or use_fp2_w2a16)
