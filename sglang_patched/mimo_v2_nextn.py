...
class MiMoV2MTP(MiMoV2ForCausalLM):
    ...
    def load_weights(self, weights: Iterable[Tuple[str, torch.Tensor]], is_nextn=False):
        ...
        deferred_qkv_scale_inv = {}  # mimo26: same deferred requant as the target loader
        for name, loaded_weight in weights:
            # mimo26: V2.6 ships 3 MTP layers but this draft model has ONE mtp_block; keep layer 0 only
            # (otherwise all three are written onto the same parameters and the last one wins)
            _m = re.match(r"model\.mtp\.layers\.(\d+)\.", name)
            if _m is not None and int(_m.group(1)) != int(os.environ.get("SGLANG_MIMO_MTP_LAYER", "0")):
                continue
        ...
        self._mimo26_resolve(params_dict, deferred_qkv_scale_inv)

    def _mimo26_resolve(self, params_dict, deferred):
        if deferred:
            _resolve_deferred_qkv_scale_inv(
                params_dict,
                deferred,
    ...
    def set_embed_and_head(self, embed, head):
        ...
        if head.dtype == torch.int8:  # mimo26: int8 target head, possibly sliced by --speculative-token-map
            import os as _os
            from sglang.srt.models.mimo_v2 import _MIMO26_LMHEAD, _Mimo26Fp8LmHeadMethod
            from sglang.srt.speculative.spec_utils import load_token_map

            scale = _MIMO26_LMHEAD["scale"].reshape(-1)
            if head.shape[0] != scale.numel():
                tm = _os.environ.get("SGLANG_MIMO_DRAFT_TOKEN_MAP", "")
                ids = load_token_map(tm).to(scale.device)
                assert ids.numel() == head.shape[0], (ids.numel(), head.shape)
                scale = scale[ids]
            self.lm_head.weight_q = head.data.contiguous()
            self.lm_head.weight_scale = scale.contiguous()
            self.lm_head.quant_method = _Mimo26Fp8LmHeadMethod()
            dv = _os.environ.get("SGLANG_MIMO_DRAFT_VOCAB", "").strip()
            if dv and head.shape[0] == _MIMO26_LMHEAD["scale"].numel():  # unsliced head + scatter-style draft vocab
                from sglang.srt.models.mimo_v2 import _Mimo26DraftSliceLmHead

                ids = torch.load(dv, map_location="cpu")["ids"].to(torch.int64).to(head.device)
                self.lm_head.quant_method = _Mimo26DraftSliceLmHead(
                    ids, self.lm_head.weight_q[ids].contiguous(), self.lm_head.weight_scale[ids].contiguous(), head.shape[0]
                )
                logger.info("mimo26: draft lm_head sliced to %d of %d rows", ids.numel(), head.shape[0])
            else:
                logger.info("mimo26: draft lm_head = int8 W8A16 over %d rows", head.shape[0])
