...
class ModelOptFp4Config(ModelOptQuantConfig):
    ...
    def get_quant_method(self, layer: torch.nn.Module, prefix: str):
        ...
        _moe_cls = ModelOptNvFp4FusedMoEMethod
        if os.environ.get("SGLANG_NVFP2_LM20", "") == "1":
            _moe_cls = ModelOptNvFp2Lm20FusedMoEMethod  # opt3: 2-bit experts via Triton
        return self._get_quant_method(
            layer,
            prefix,
...
# =============================================================================
# NVFP2 (lm20 codebook) experts, W2A16 via the Triton fused-MoE kernel.
# Added by ~/nvfp2/opt3/nvfp2_kernel/patch_sglang_nvfp2.py. Enabled with env
# SGLANG_NVFP2_LM20=1 together with --quantization modelopt_fp4 and
# --moe-runner-backend triton. Checkpoint layout per expert (same names as NVFP4):
#   weight         uint8 [N, K//4]   2-bit codes, 4/byte along K, LSB first
#   weight_scale   e4m3  [N, K//16]
#   weight_scale_2 f32   []
#   input_scale    f32   []          (ignored: activations stay bf16)
# =============================================================================
class ModelOptNvFp2Lm20FusedMoEMethod(ModelOptNvFp4FusedMoEMethod):
    PACK = 4
    ...
    @property
    def load_up_proj_weight_first(self) -> bool:
        return False  # Triton kernel wants [gate; up]; loader reads this via getattr
    ...
    def create_weights(
        ...
        **extra_weight_attrs,
        ...
        layer.register_parameter(
            "w13_weight",
            _w((E, num_shards * intermediate_size_per_partition, hidden_size // self.PACK)))
        layer.register_parameter(
            "w2_weight",
            _w((E, hidden_size, intermediate_size_per_partition // self.PACK)))
        layer.register_parameter(
            "w13_weight_scale",
            _s((E, num_shards * intermediate_size_per_partition, hidden_size // gs)))
        layer.register_parameter(
            "w2_weight_scale",
            _s((E, hidden_size, intermediate_size_per_partition // gs)))
        layer.w13_blockscale_swizzled = None
        layer.w2_blockscale_swizzled = None
        ...
        w13_s2_shape = (E, 2) if layer.moe_runner_config.is_gated else (E,)
        layer.register_parameter(
            "w13_weight_scale_2",
            PerTensorScaleParameter(data=torch.empty(w13_s2_shape, dtype=torch.float32),
                                    weight_loader=weight_loader))
        layer.register_parameter(
            "w2_weight_scale_2",
            PerTensorScaleParameter(data=torch.empty(E, dtype=torch.float32),
                                    weight_loader=weight_loader))
        extra_weight_attrs.update({"quant_method": FusedMoeWeightScaleSupported.TENSOR.value})
        w13_input_scale = _make_per_tensor_scale_parameter(
            (layer.num_experts, num_shards), weight_loader=weight_loader, fill_value=1.0)
        w13_input_scale._sglang_require_global_experts = True
        layer.register_parameter("w13_input_scale", w13_input_scale)
        w2_input_scale = _make_per_tensor_scale_parameter(
            (layer.num_experts,), weight_loader=weight_loader, fill_value=1.0)
        w2_input_scale._sglang_require_global_experts = True
        layer.register_parameter("w2_input_scale", w2_input_scale)

    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        """Keep the e4m3 block scales as stored (exact); expand the fp32 per-tensor scale
        to a per-(expert, output-row) vector applied in the kernel epilogue (exact)."""
        E, twoN, _ = layer.w13_weight_scale.shape
        s2 = layer.w13_weight_scale_2.data.to(torch.float32)
        g13 = torch.empty(E, twoN, dtype=torch.float32, device=s2.device)
        if s2.dim() == 2 and s2.shape[1] == 2:
            N = twoN // 2
            g13[:, :N] = s2[:, 0:1]
            g13[:, N:] = s2[:, 1:2]
        else:
            g13[:] = s2.view(E, 1)
        layer.w13_gscale = Parameter(g13.contiguous(), requires_grad=False)
        hidden = layer.w2_weight_scale.shape[1]
        g2 = layer.w2_weight_scale_2.data.to(torch.float32).view(E, 1).expand(E, hidden)
        layer.w2_gscale = Parameter(g2.contiguous(), requires_grad=False)
        ...
        logger.info(
            "NVFP2-lm20 MoE ready: w13 %s w2 %s (uint8, 4 codes/byte), e4m3 scales %s / %s, fp32 gscale %s / %s",
            tuple(layer.w13_weight.shape), tuple(layer.w2_weight.shape),
            tuple(layer.w13_weight_scale.shape), tuple(layer.w2_weight_scale.shape),
            tuple(layer.w13_gscale.shape), tuple(layer.w2_gscale.shape))

    def create_moe_runner(self, layer: torch.nn.Module, moe_runner_config: MoeRunnerConfig):
        self.moe_runner_config = moe_runner_config
        self._moe_runner_backend = MoeRunnerBackend.TRITON
        self.runner = MoeRunner(MoeRunnerBackend.TRITON, moe_runner_config)
    ...
    def apply(self, layer, dispatch_output):
        ...
        quant_info = TritonMoeQuantInfo(
            ...
            w2_weight=layer.w2_weight,
            use_fp2_w2a16=True,
            w13_scale=layer.w13_weight_scale,   # e4m3 [E, 2N, K/16]
            w2_scale=layer.w2_weight_scale,     # e4m3 [E, K, N/16]
            w13_zp=layer.w13_gscale,            # fp32 [E, 2N]  (zp slot reused)
            w2_zp=layer.w2_gscale,              # fp32 [E, K]
