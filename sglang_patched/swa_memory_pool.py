...
class SWAKVPool(BaseSWAKVPool):
    ...
    def set_kv_buffer(
        ...
        if is_swa_layer:
            ...
            if swa_loc is None:
                # mimo26: the multi-step DRAFT decode path of the triton backend passes no pre-translated
                # SWA loc (MiMo's MTP block is an SWA layer). Translate here: full_to_swa_index_mapping is a
                # persistent tensor, so the captured gather reads live contents on replay.
