...
class TritonAttnBackend(AttentionBackend):
    ...
    def __init__(
        ...
        self.use_verify_splitkv = (  # mimo26: gfx95-only gate lifted; the kernel is plain Triton
    ...
    def _update_draft_extend_buffers(
        ...
        # mimo26: SWA window over the prefix for the draft-extend graph (mirrors the eager
        # extend path, which builds it from extend_prefix_lens). Out-of-graph, refilled per replay.
        self._mimo26_de_window = None
        if self.sliding_window_size is not None and self.sliding_window_size > 0:
            w_indptr, w_indices, _, w_off = update_sliding_window_buffer(
                self.window_kv_indptr,
                self.req_to_token,
                self.sliding_window_size,
                kv_lens,
            ...
            self.cuda_graph_window_kv_offsets[:bs] = w_off
            self._mimo26_de_window = (w_indptr, w_indices, self.cuda_graph_window_kv_offsets[:bs])
    ...
    def init_forward_metadata_out_graph(
        ...
        if in_capture:
            ...
            if forward_mode.is_decode_or_idle() and spec_info is not None:
                win_indptr, win_indices, mimo26_win = self.window_kv_indptr, None, False
                if self.sliding_window_size is not None and self.sliding_window_size > 0:
                    # mimo26: persistent window buffers, recomputed in-graph (init_forward_metadata_in_graph)
                    self._mimo26_ensure_spec_window_bufs(spec_info.kv_indptr.shape[0] - 1)
                    win_indptr, win_indices = self._mimo26_spec_window(
                        spec_info.kv_indptr, spec_info.kv_indices, self._mimo26_win_indptr, self._mimo26_win_indices
                    )
                    mimo26_win = True
                self.forward_metadata = ForwardMetadata(
                    ...
                    custom_mask=None,
                    mask_indptr=None,
                    window_kv_indptr=win_indptr,
                    window_kv_indices=win_indices,
                    window_num_kv_splits=None,
                    window_kv_offsets=None,
                    swa_attn_logits=self.cuda_graph_swa_attn_logits,
                )
                self.forward_metadata._mimo26_spec_window = mimo26_win
                return
    ...
    # ---- mimo26: SWA window metadata for the multi-step DRAFT decode path -------------------
    # The draft steps read spec_info.kv_indptr / kv_indices (full-pool ids for the whole sequence,
    # refilled in place before every replay). MiMo's MTP block is a sliding-window layer, so it
    # needs the LAST min(len, W) of those ids translated to the SWA pool. Everything here is plain
    # static-shape torch on persistent buffers, so it can be recorded in the draft CUDA graph.
    def _mimo26_spec_window(self, kv_indptr, kv_indices, indptr_out, indices_out):
        W = self.sliding_window_size
        bs = kv_indptr.shape[0] - 1
        lens = kv_indptr[1:] - kv_indptr[:-1]
        wl = torch.clamp(lens, max=W)
        indptr_out[: bs + 1].zero_()  # device-side, capture-safe (no host scalar copy)
        indptr_out[1 : bs + 1] = torch.cumsum(wl, 0).to(indptr_out.dtype)
        pos = torch.arange(bs * W, device=kv_indptr.device)
        req = pos // W
        off = pos % W
        valid = off < wl[req]
        src = kv_indptr[req + 1].long() - wl[req].long() + off
        src = torch.where(valid, src, torch.zeros_like(src))
        # kv_indices is a view sized for the CAPTURE batch; at replay the live indptr reaches past it.
        # Gather from the underlying buffer (same storage, from this view's offset to the end).
        if kv_indices.is_contiguous():
            n_full = kv_indices.untyped_storage().nbytes() // kv_indices.element_size() - kv_indices.storage_offset()
            kv_full = torch.empty(0, dtype=kv_indices.dtype, device=kv_indices.device).set_(
                kv_indices.untyped_storage(), kv_indices.storage_offset(), (n_full,)
            )
        else:
            kv_full = kv_indices
        vals = kv_full[src]
        if hasattr(self.token_to_kv_pool, "translate_loc_from_full_to_swa"):
            vals = self.token_to_kv_pool.translate_loc_from_full_to_swa(vals)
        dst = indptr_out[req].long() + off
        dst = torch.where(valid, dst, torch.full_like(dst, indices_out.shape[0] - 1))  # dump slot
        indices_out[dst] = vals.to(indices_out.dtype)
        return indptr_out[: bs + 1], indices_out

    def _mimo26_ensure_spec_window_bufs(self, bs):
        W = self.sliding_window_size
        if getattr(self, "_mimo26_win_indptr", None) is None or self._mimo26_win_indptr.shape[0] < bs + 1:
            self._mimo26_win_indptr = torch.zeros(bs + 1, dtype=torch.int32, device=self.device)
            self._mimo26_win_indices = torch.zeros(bs * W + 1, dtype=torch.int64, device=self.device)

    def init_forward_metadata_in_graph(self, forward_batch: ForwardBatch):
        md = getattr(self, "forward_metadata", None)
        if md is not None and getattr(md, "_mimo26_spec_window", False):
            self._mimo26_spec_window(md.kv_indptr, md.kv_indices, self._mimo26_win_indptr, self._mimo26_win_indices)
    ...
    def init_forward_metadata(self, forward_batch: ForwardBatch):
        ...
        if forward_batch.forward_mode.is_decode_or_idle():
            if spec_info is None or spec_info.kv_indptr is None:
                ...
                bs = kv_indptr.shape[0] - 1
                if self.sliding_window_size is not None and self.sliding_window_size > 0:
                    # mimo26: eager multi-step draft decode needs the SWA window too
                    window_kv_indptr, window_kv_indices = self._mimo26_spec_window(
                        kv_indptr, kv_indices,
                        torch.zeros(bs + 1, dtype=torch.int32, device=self.device),
                        torch.zeros(bs * self.sliding_window_size + 1, dtype=torch.int64, device=self.device),
                    )
    ...
    def _build_cuda_graph_forward_metadata(
        ...
        if forward_mode.is_decode_or_idle():
            ...
            return ForwardMetadata(
                ...
                mask_indptr=None,
                window_kv_indptr=(
                    self._mimo26_de_window[0] if getattr(self, "_mimo26_de_window", None) else self.window_kv_indptr
                ),
                window_kv_indices=(
                    self._mimo26_de_window[1] if getattr(self, "_mimo26_de_window", None) else None
                ),
                window_num_kv_splits=None,
                window_kv_offsets=(
                    self._mimo26_de_window[2] if getattr(self, "_mimo26_de_window", None) else None
                ),
                swa_out_cache_loc=swa_out_cache_loc,
                out_cache_loc_full_physical=out_cache_loc_full_physical,
            )
