...
def _get_ckpt_qkv_shard_sizes(config, layer_name, ckpt_tp):
    ...
    if m is None and "mtp_block" not in layer_name:
        return None
    if m is None:
        is_swa = True  # mimo26: the MTP block is an SWA layer (mimo_v2_nextn builds it from the swa_* fields)
    else:
...
# ---- mimo26: load-time per-channel FP8 lm_head (env SGLANG_MIMO_LMHEAD_FP8=1) -------------------------
# Ported from opt3/fp8_dense/qwen4_exp_fp8.py. The 152576 x 4096 BF16 lm_head is 1.25 GB per decode step
# (~12 % of the step's bytes). Weight -> e4m3 with fp32 per-row scale; activations quantized per token.
# The MTP draft shares this Parameter via set_embed_and_head, so the scale + method ride on the Parameter.
class _Mimo26Fp8LmHeadMethod:
    def process_weights_after_loading(self, layer):
        return None

    def apply(self, layer, x, bias=None):
        # W8A16 (exact bf16 activations, fp8 weights dequantised in registers) for decode-size batches;
        # per-token fp8 activations only for large M (logprob prefill), where the tie-flip risk is moot.
        if x.shape[0] <= 16 and x.is_contiguous():
            from sglang.srt.models.mimo26_w8a16 import w8a16_linear

            out = w8a16_linear(x, layer.weight_q, layer.weight_scale.reshape(-1))
            return out if bias is None else out + bias
        # large M (logprob prefill): dequantise int8 rows on the fly, bf16 GEMM
        w = (layer.weight_q.to(torch.float32) * layer.weight_scale).to(x.dtype)
...
class _Mimo26DraftSliceLmHead:
    """Draft-only lm_head: logits for the hot-vocabulary rows (SGLANG_MIMO_DRAFT_VOCAB=<ids.pt>), scattered into a
    full-width tensor at dtype-min. Greedy verify keeps a draft token only if it equals the target argmax, so this
    changes proposals never acceptances (opt3 roadmap item 9; Qwen: 26 % of vocab kept acceptance, +5-10 % tok/s)."""

    def __init__(self, ids, wq, scale, V):
        self.ids, self.wq, self.scale, self.V = ids, wq, scale, V
    ...
    def apply(self, layer, x, bias=None):
        ...
        if x2.shape[0] <= 16:
            part = w8a16_linear(x2, self.wq, self.scale)
        else:
            part = torch.matmul(x2, (self.wq.to(torch.float32) * self.scale[:, None]).to(x2.dtype).t())
        out = torch.full((x2.shape[0], self.V), torch.finfo(part.dtype).min, dtype=part.dtype, device=part.device)
        out[:, self.ids] = part
        return out if bias is None else out + bias


_MIMO26_LMHEAD = {"q": None, "scale": None}


def _mimo26_install_fp8_lm_head(lm_head, head_param, draft=False):
    import os as _os

    lm_head.weight_scale = head_param._mimo26_scale
    lm_head.weight_q = head_param._mimo26_q  # [N, K] int8 contiguous for the W8A16 kernel (same storage as weight.t())
    path = _os.environ.get("SGLANG_MIMO_DRAFT_VOCAB", "").strip()
    if draft and path:
        ids = torch.load(path, map_location="cpu")["ids"].to(torch.int64).to(lm_head.weight_q.device)
        wq = lm_head.weight_q[ids].contiguous()
        sc = lm_head.weight_scale.reshape(-1)[ids].contiguous()
        lm_head.quant_method = _Mimo26DraftSliceLmHead(ids, wq, sc, lm_head.weight_q.shape[0])
        logger.info("mimo26: draft lm_head sliced to %d of %d rows (%.0f MB)", ids.numel(), lm_head.weight_q.shape[0], wq.numel() / 1e6)
        return
    lm_head.quant_method = _Mimo26Fp8LmHeadMethod()


def _mimo26_maybe_fp8_lm_head(model):
    import os as _os

    if _os.environ.get("SGLANG_MIMO_LMHEAD_FP8", "0") != "1" or getattr(model, "lm_head", None) is None:
        return
    w = model.lm_head.weight.data
    if w.dtype not in (torch.bfloat16, torch.float16):
        return
    # int8 per-row (7-bit mantissa): fp8 e4m3 rows flipped near-tie GSM8K answers (96 -> 93/94)
    wf = w.float()
    s = wf.abs().amax(dim=1, keepdim=True).clamp(min=1e-12) / 127.0
    q = torch.round(wf / s).clamp(-127, 127).to(torch.int8).contiguous()
    del wf
    p = torch.nn.Parameter(q.t(), requires_grad=False)  # [K, N] view, as apply_fp8_linear wants
    p._mimo26_scale = torch.nn.Parameter(s.to(torch.float32).contiguous(), requires_grad=False)
    p._mimo26_q = q  # the contiguous [N, K] tensor itself (no copy)
    _MIMO26_LMHEAD["q"] = q
    _MIMO26_LMHEAD["scale"] = p._mimo26_scale
    model.lm_head.weight = p
    _mimo26_install_fp8_lm_head(model.lm_head, p)
    torch.cuda.empty_cache()
    logger.info("mimo26: lm_head -> per-row INT8 W8A16 (%s), freed %.2f GB", tuple(q.shape), (w.numel() * 2 - q.numel()) / 1e9)
...
class MiMoV2ForCausalLM(nn.Module, AudioEncoderMixin):
    ...
    def __init__(
        ...
        # mimo26: the V2.6 config class turns a missing vision/audio block into {}, not None
    ...
    def load_weights(self, weights: Iterable[Tuple[str, torch.Tensor]]):
        ...
        _mimo26_maybe_fp8_lm_head(self)
        from sglang.srt.models.mimo26_nvfp4 import _mimo26_dense_to_nvfp4, _mimo26_install_dense_calib

        if not _mimo26_install_dense_calib(self):  # env SGLANG_MIMO_DENSE_CALIB_DIR
            _mimo26_dense_to_nvfp4(self)  # env SGLANG_MIMO_DENSE_NVFP4=qkv,o

    def get_embed_and_head(self):
        assert (
            self.model is not None and self.lm_head is not None
        ), "get_embed_and_head() is not available in encoder_only mode"
        if _MIMO26_LMHEAD["q"] is not None:  # int8 lm_head: the worker slices dim 0 (hot token map) and the draft rebuilds W8A16
            return self.model.embed_tokens.weight, torch.nn.Parameter(_MIMO26_LMHEAD["q"], requires_grad=False)
